...
- Machine translation requires `TRANSLATION_PROVIDER` plus credentials (`OPENAI_API_KEY` for `openai`, or `TRANSLATION_API_KEY` for `google`/`deepl`). Prefer `openai` or `google` when `uz` is a target; DeepL has no Uzbek.
