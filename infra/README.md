...
| `SMS_PROVIDER`, `ESKIZ_EMAIL`, `ESKIZ_PASSWORD` | optional (`eskiz`) | SMS one-time codes for phone sign-in. Without them the phone route still works through the Telegram bot's contact share (free, verified by Telegram) |
...
> **Railway requirement — cap indexing memory:** Meilisearch sizes its indexing budget from the *host's* RAM (it is cgroup-unaware), so on shared Railway hosts the first write task tries to allocate far more than the service limit and the container dies in a silent OOM/SIGKILL crash loop (it also resumes the failing task on restart, so the loop never ends). Always set `MEILI_MAX_INDEXING_MEMORY=1GiB` and `MEILI_MAX_INDEXING_THREADS=2` on the service. With those set, the staging setup is: image `getmeili/meilisearch:v1.13`, `MEILI_MASTER_KEY`, `MEILI_ENV=production`, `MEILI_NO_ANALYTICS=true`, `MEILI_DB_PATH=/data/data.ms`, volume mounted at `/data` (index persists across restarts). Without a volume it also runs fine; the API's self-healing reindex rebuilds the index after each restart.
